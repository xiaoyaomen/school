...
var cmg = {
        ...
        if (cmg._strokeclear()) {
            ...
            if (this._directionChain && !cmg._toCancelGesture) {
                ...
                if (cmg._toCancelGesture) {
                    ...
                    chrome.runtime.sendMessage({
                        type: "gaEvent",
                        gaCategory: "Cancel Gesture",
                        gaAction: cmg._toCancelGestureType === "trash" ? "Trash bin" : "Esc",
                        gaLabel: action !== "" ? `${action}-${location.origin}` : "null"
                    });
    ...
    _dragAction: function(e) {
        ...
        if (!cmg.clickedOptin) {
            chrome.runtime.sendMessage({
                type: "openMakeASelectionOverlay"
            });
            return;
        }
    ...
    _gestureaction: function(act, whitelist) {
        ...
        if (location.origin !== "chrome-extension://ebbnmoppemnnpjbgjhdgnlcbempodpdc") {
            chrome.runtime.sendMessage({
                type: "gaEvent",
                gaCategory: "Main KPIs",
                gaAction: "Gesture completed",
                gaLabel: act + "-" + location.origin
            });
            chrome.runtime.sendMessage({
                type: "shouldShowRateUsOverlay"
            });
        }
